...
{
    internal partial class FrmGrowCtlTemp : Form
    {
        public FrmGrowCtlTemp()
        {
            InitializeComponent();
            Load += Timer1_Tick;
        }
        private void FrmGrowCtlTemp_Load(object sender, EventArgs e)
        {
        ...
        private void Timer1_Tick(object sender, EventArgs e)
        {
            if (PLC.Count == 2)
            {
                ...
                LabOut.Text = Math.Round(PLC.ReadValue[11], 1).ToString("f1");
                ...
                if (BtnLimit.BackColor == Color.Fuchsia)
                {
                    ...
                    LabIntegVal.Text = Math.Round(PLC.ReadValue[13], 1).ToString("f1");
                    LabDifferVal.Text = Math.Round(PLC.ReadValue[14], 1).ToString("f1");
                }
                ...
                LabOut.BackColor = PLC.ReadStatus[3] == 1 || PLC.ReadStatus[5] == 1 ? Color.Gray : Color.White;
            }
        }
        ...
        private void LabOut_Click(object sender, EventArgs e)
        {
            ...
            {
                KeyForm kf = new KeyForm();
                kf.ImportData(ref LabOut, 0, 2000);
                kf.ShowDialog();
                if (kf.DialogResult == DialogResult.OK)
                {
                    PLC.SendFloat(33, 40, Convert.ToSingle(LabOut.Text));
                    if (PLC.ReadStatus[5] == 1)
                    {
                        Record.WriteIn("热场温度设定", LabOut.Text, "Yellow");
                    }
                }
            }
        }
        ...
        private void LabDifferVal_Click(object sender, EventArgs e)
        {
            if (LabDifferVal.BackColor == Color.White)
            {
                ...
                if (BtnLimit.BackColor == Color.Fuchsia)
                {
                    kf.ImportData(ref LabDifferVal, 0, 2000);
                }
                kf.ShowDialog();
                if (kf.DialogResult == DialogResult.OK)
                {
                    ...
                    if (BtnLimit.BackColor == Color.Fuchsia)
                    {
                        PLC.SendFloat(33, 52, Convert.ToSingle(LabDifferVal.Text));
        ...
        private void LabIntegVal_Click(object sender, EventArgs e)
        {
            if (LabIntegVal.BackColor == Color.White)
            {
                ...
                if (BtnLimit.BackColor == Color.Fuchsia)
                {
                    kf.ImportData(ref LabIntegVal, 0, 2000);
                }
                kf.ShowDialog();
                if (kf.DialogResult == DialogResult.OK)
                {
                    ...
                    if (BtnLimit.BackColor == Color.Fuchsia)
                    {
                        PLC.SendFloat(33, 48, Convert.ToSingle(LabIntegVal.Text));
